...
export const useAI = () => {
    ...
    const findSimilarCompanies = useCallback((company, allCompanies) => {
    ...
    const analyzeMarketTrends = useCallback((companies) => {
        ...
        return {
            ...
            categoryDistribution: Object.fromEntries(topCategories.map(([cat, count]) => [
                cat,
    ...
    // Anomaly Detection
    const detectAnomalies = useCallback((company) => {
        const anomalies = [];
    ...
    const analyzeCompetitiveLandscape = useCallback((company, competitors) => {
        const analysis = {
            marketPosition: '',
            strengths: [],
            weaknesses: [],
        ...
        const competitorCategories = new Set(competitors.flatMap(c => c.category));
        const uniqueCategories = company.category.filter(cat => !competitorCategories.has(cat));
